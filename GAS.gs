...
// Self-heal: if the same transaction id appears on multiple rows (left over from the old
// non-deterministic recurring-id bug), keep the first row and remove the rest. Runs on every push.
function removeDupIdRows(sh) {
  ...
  ids.forEach(function(r,i){
    ...
    if(!id) return;
    if(seen[id]) toDelete.push(i);
    else seen[id] = true;
  });
  if(!toDelete.length) return;
  toDelete.sort(function(a,b){ return b-a; });
  toDelete.forEach(function(i){ sh.deleteRow(i+2); });
}
